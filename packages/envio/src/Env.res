...
let processingBatchSize =
  envSafe->EnvSafe.get("ENVIO_PROCESSING_BATCH_SIZE", S.option(S.int->S.intMin(1)))
...
//
// These were environment-tunable on the 3.9.0-reptilian line. The knobs are
// dropped in the 3.12.0 port: ENVIO_SOURCE_QUERY_RETRY_TIMEOUT_MILLIS had no
// reference anywhere in chain-indexer or its host, and
// ENVIO_SOURCE_QUERY_MAX_RETRIES appeared only in a runbook line and a comment.
// The bounding behaviour is retained at the values the knobs defaulted to; make
// them configurable again only when something actually needs to tune them.
let maxSourceQueryRetries = 200
let sourceQueryRetryTimeoutMillis = 600_000
