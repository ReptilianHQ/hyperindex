  ...
  let make = (
    ...
    for idx in 0 to partitions->Array.length - 1 {
      ...
      {selection: {dependsOnAddresses: false}} =>
        newPartitions->Array.push(p)->ignore
      | {dynamicContract: Some(contractName)} =>
        // Cross-contract coalescing guard: only merge a partition into the
        // optimised set when all its addresses belong to a single contract.
        // A partition that mixes contract addresses (possible when two dynamic
        // contracts share an address space) must be kept intact.
