        ...
        match (request.method.as_str(), path) {
            ...
            ("POST", QUERY_PATH) => {
                let (spec, height) = {
                    ...
                    // Upstream records the decoded body string; the fork's
                    // filterByQuery peek is kept on top of it.
                ...
                {
                    if seed > 0 {
                        // `request.body` was a String on the 3.9.0 base, where
                        // `.bytes()` yielded its bytes. It is a Vec<u8> on
                        // 3.12.0, so iterate it directly for the same fold.
                        let delay = request
                            .body
                            .iter()
