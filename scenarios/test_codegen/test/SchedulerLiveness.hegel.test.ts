...
const envioSrc = new URL("../node_modules/envio/src/", import.meta.url);
...
const hasWork = (p: any, head: number) =>
  p.latestFetchedBlock.blockNumber < Math.min(head, p.mergeBlock ?? head);
...
const land = (sim: Sim, index: number) => {
  ...
  sim.fs = FetchState.handleQueryResult(
    sim.fs,
    raw,
    { blockNumber: Math.min(toBlock, sim.head), blockTimestamp: 0 },
    [],
  );
};

const allAtHead = (sim: Sim) =>
  partitionsOf(sim.fs).every((p) => p.latestFetchedBlock.blockNumber >= sim.head);
  ...
  while (!allAtHead(sim)) {
    if (sim.ticks >= maxTicks) {
      const frontiers = partitionsOf(sim.fs)
        .map((p) => `${p.id}@${p.latestFetchedBlock.blockNumber}${p.mergeBlock !== undefined ? `->${p.mergeBlock}` : ""}`)
        .join(" ");
